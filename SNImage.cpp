#include"SNImage.h"
//VERSION POUR BMP4 Taille entête 0x8A généré par ImageMagic (au lieu de 0x36 généré par paint 24 bits)
SNImage::SNImage()
...
compteur+=4;
	entree.read((char*)&zero,sizeof(int));
compteur+=4;     //ici le compteur doit être à debutImage
	//cout<<hex<<compteur<<endl;cin.get();
	while(compteur<debutImage)
	{	unsigned char octet;
		entree.read((char*)&octet,sizeof(char));
		compteur++;
	}
	//debutImage=0x36;
	//cout<<hex<<compteur<<endl;cin.get();
...
{   ofstream sortie; int i,j; char k,z=0,n;
	...
	sortie.write((char*)&zero,sizeof(int));
	if(debutImage==0x8A)
	for(int zz=0;zz<(0x8A-0x36);zz++)sortie.write((char*)&z,sizeof(char));
...
Coordonnee SNImage::RechercherZoneDiagonale(int l,int h,Pixel couleur)
{   Coordonnee coord={-1,-1};
	int i,j,m,n;
	bool zoneTrouve;
	float coef = (double)largeur/(double)hauteur;
	for(int i=0;i<hauteur;i++)
	{	for(int j=0;j<coef*i;j++)
		{   if(image[i][j].bleu==couleur.bleu && image[i][j].vert==couleur.vert && image[i][j].rouge==couleur.rouge)
			...
			// Pour dessiner la zone recherchée
			//image[i][j].bleu = 230;
			//image[i][j].vert = 230;
			//image[i][j].rouge = 230;
...
Coordonnee SNImage::RechercherZone(int l,int h,int bleu,int vert,int rouge, float pourcentErreur)
{   Coordonnee coord={-1,-1};
	int i,j,m,n;
	int minB=bleu-bleu*pourcentErreur/100,maxB=bleu+bleu*pourcentErreur/100;
	int minV=vert-vert*pourcentErreur/100,maxV=vert+vert*pourcentErreur/100;
	int minR=rouge-rouge*pourcentErreur/100,maxR=rouge+rouge*pourcentErreur/100;
	if(minB<0) minB=0; if(minV<0) minV=0; if(minR<0) minR=0;
	if(maxB>255) maxB=255; if(maxV>255) maxV=255; if(maxR>255) maxR=255;
	bool zoneTrouve;
	for(int i=0;i<hauteur;i++)
	{	for(int j=0;j<largeur;j++)
		{   if(image[i][j].bleu>minB&&image[i][j].bleu<maxB
			 &&image[i][j].vert>minV&&image[i][j].vert<maxV
			 &&image[i][j].rouge>minR&&image[i][j].rouge<maxR)
			 {   zoneTrouve=true;
				for(int m=0;m<h;m++)
				{   for(int n=0;n<l;n++)
					{   if(!(image[i+m][j+n].bleu>minB&&image[i+m][j+n].bleu<maxB
							&&image[i+m][j+n].vert>minV&&image[i+m][j+n].vert<maxV
							&&image[i+m][j+n].rouge>minR&&image[i+m][j+n].rouge<maxR))
							zoneTrouve=false;
					}
				}
				if(zoneTrouve)
...
Coordonnee SNImage::RechercherZoneLaPlusProche(int l,int h,Pixel couleur)
{       Coordonnee pt[50];int min,imin;
		min=Largeur()+Hauteur(),imin=0;
		for(int p=0;p<h;p++)
		{	pt[p]=RechercherZone(l,h,couleur);
			//la ligne est alors effacée
			Colorier(0,pt[p].ligne,Largeur(),1,0xFFFFFF);
			if(pt[p].ligne+pt[p].colonne<min) {min=pt[p].ligne+pt[p].colonne;imin=p;}
		}
		return pt[imin];
}
Coordonnee SNImage::RechercherZoneLaPlusProche(int l,int h,int bleu,int vert,int rouge, float pourcentErreur)
{       Coordonnee pt[50];int min,imin;
		min=Largeur()+Hauteur(),imin=0;
		for(int p=0;p<h;p++)
		{	pt[p]=RechercherZone(l,h,bleu,vert,rouge,pourcentErreur);
			//la ligne est alors effacée
			Colorier(0,pt[p].ligne,Largeur(),1,0xFFFFFF);
			if(pt[p].ligne+pt[p].colonne<min) {min=pt[p].ligne+pt[p].colonne;imin=p;}
		}
...
Coordonnee SNImage::RechercherIlot(Coordonnee depart,int largeurContour, Pixel couleurContour, Pixel couleurcentre)
{   Coordonnee coord={-1,-1};
	int i,j,m;
	bool zoneTrouve;
	for(int i=depart.ligne;i<hauteur-largeurContour;i++)
	{	for(int j=depart.colonne;j<largeur-largeurContour;j++)
		{   if(image[i][j].bleu==couleurContour.bleu && image[i][j].vert==couleurContour.vert && image[i][j].rouge==couleurContour.rouge)
			{   zoneTrouve=true;
				for(int m=0;m<largeurContour;m++)
				{	if(image[i][j+m].bleu!=couleurContour.bleu || image[i][j+m].vert!=couleurContour.vert || image[i][j+m].rouge!=couleurContour.rouge) zoneTrouve=false;
					if(image[i+m][j].bleu!=couleurContour.bleu || image[i+m][j].vert!=couleurContour.vert || image[i+m][j].rouge!=couleurContour.rouge) zoneTrouve=false;
					if(image[i+largeurContour-1][j+m].bleu!=couleurContour.bleu || image[i+largeurContour-1][j+m].vert!=couleurContour.vert || image[i+largeurContour-1][j+m].rouge!=couleurContour.rouge) zoneTrouve=false;
					if(image[i+m][j+largeurContour-1].bleu!=couleurContour.bleu || image[i+m][j+largeurContour-1].vert!=couleurContour.vert || image[i+m][j+largeurContour-1].rouge!=couleurContour.rouge) zoneTrouve=false;
				}
				if(zoneTrouve)
				{   if(image[i+largeurContour/2][j+largeurContour/2].bleu==couleurcentre.bleu && image[i+largeurContour/2][j+largeurContour/2].vert==couleurcentre.vert && image[i+largeurContour/2][j+largeurContour/2].rouge==couleurcentre.rouge)
					{	coord.ligne=i;
						coord.colonne=j;
						i=hauteur;
						j=largeur;
